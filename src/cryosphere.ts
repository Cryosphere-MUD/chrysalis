...
export function handleTable(data: any) {
  ...
  if (data.title && loose) {
    ...
    const children = parseANSI(data.title);
    if (children)
  ...
  if (data.topnotes && !loose) {
    ...
    const children = parseANSI(topnote);
    if (children)
      topCell.replaceChildren(children);
  ...
  data.data.forEach((row: any) => {
    if (row) {
      ...
      row.forEach((cell: any, idx: number) => {
        ...
        const children = parseANSI(text);
        if (children)
          newCell.replaceChildren(children);
